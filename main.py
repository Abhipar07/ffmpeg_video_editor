...
def validate_file_size(file: UploadFile) -> bool:
    return not (hasattr(file, 'size') and file.size and file.size > MAX_FILE_SIZE)
...
def convert_webp_to_png(image_path: Path) -> Path:
    ...
    result = subprocess.run(["ffmpeg", "-y", "-i", str(image_path), str(new_path)],
                            capture_output=True, text=True, timeout=60)
    if result.returncode != 0:
        logger.error(f"WebP to PNG conversion failed: {result.stderr}")
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, img_path in enumerate(image_paths):
                new_path = temp_path / f"img_{i:04d}{img_path.suffix}"
...
async def create_video(
    ...
    try:
        ...
        success = create_video_from_images(image_paths,
